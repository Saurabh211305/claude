    ...
    <section className="bg-paper py-28 text-ink md:py-40">
      <div className="container-fluid">
        <div className="max-w-2xl">
          <Reveal>
            <p className="text-eyebrow mb-6 text-gold">Why Trixis Homes</p>
          </Reveal>
        ...
        <StaggerGroup className="mt-16 grid grid-cols-1 gap-px overflow-hidden rounded-sm bg-ink/10 sm:grid-cols-2 lg:grid-cols-3">
          {VALUES.map((value) => (
            <StaggerItem key={value.title}>
              <div className="group h-full bg-paper p-10 transition-colors duration-500 hover:bg-ink">
                <value.icon
                  className="text-gold transition-transform duration-500 group-hover:-translate-y-1 group-hover:scale-110"
                  size={30}
                  strokeWidth={1.25}
                />
                <p className="font-display mt-6 text-xl text-ink transition-colors duration-500 group-hover:text-paper">
                  {value.title}
                </p>
                <p className="mt-3 text-sm leading-relaxed text-ink/60 transition-colors duration-500 group-hover:text-paper/60">
                  {value.description}
                </p>
              </div>
            </StaggerItem>
